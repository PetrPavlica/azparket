...
class ImageStorage
{
	...
	public function delete($arg)
	{
		...
		$pattern = preg_replace('/__file__/', preg_quote($script->name), ImageNameScript::PATTERN);
    ...
    /**
     * Delete cached images and all thumbnails/resized images, etc
     * @param  mixed $arg
     * @return void
     */
    public function deleteCache($arg)
    {
        if (is_object($arg) && $arg instanceof Image) {
            $script = ImageNameScript::fromIdentifier($arg->identifier);
        } else {
        ...
        foreach (new \DirectoryIterator($dir) as $file_info) {
            if (preg_match($pattern, $file_info->getFilename()) && $script->name.'.'.$script->extension != $file_info->getFilename()) {
                unlink($file_info->getPathname());
            }
        }
    }
	...
	public function fromIdentifier($args)
	{
		 ...
		 */
		$flag = isset($args[2]) && $args[2] ? $args[2] : $this->default_transform;
		$quality = isset($args[3]) && $args[3] ? $args[3] : $this->quality;
		$watermark = isset($args[4]) ? $args[4] : $this->watermark;
		...
		if (!file_exists(implode('/', [$this->data_path, $identifier]))) {
            ...
            if ($watermark) {
                $wm = Nette\Utils\Image::fromFile('front/img/vodoznak.png');
                $_image->place($wm, '50%', '50%');
            }

			$_image->resize($size[0], $size[1], $flag);

            if (strtolower($script->extension) != 'png') {
                $_image->sharpen();
            }
